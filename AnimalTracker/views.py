...
def get_gps_data():
    ...
    for index, row in df.iterrows():
        ...
        gps_data.append({'timestamp': timestamp, 'date': str(date), 'time': str(time), 'lat': lat, 'lng': lon})
...
def home():
    ...
    if user_found:
        return render_template("homepage.html", gps_data=gps_data, lat=lat, lng=lng, rad=rad)
...
def back():
    gps_data = get_gps_data()
    return render_template("homepage.html", gps_data=gps_data, lat=lat_gb, lng=lng_gb, rad=rad_gb)
